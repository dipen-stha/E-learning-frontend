...
import { UserCourseState, UserCourseDetail, UserCourseStats } from "@/services/types/Common";
...
export const useUserCourseStore = create<UserCourseState>((set, get) => ({
    ...
    setUserStats: (userStats: UserCourseStats) => set({userCourseStats: userStats}),
    ...
    fetchUserCourseStats: async(userId: number) => {
        try{
            const response = await api.get(UserCourseAPI.fetchUserCourseStats(userId))
            if(response.data) {
    ...
    fetchUserCourseByCourse: async(courseId: number) => {
        try{
            const response = await api.get(UserCourseAPI.fetchUserCourseByCourse(courseId))
            if(response.data){
                get().setUserCourseItem(response.data)
